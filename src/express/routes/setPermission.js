...
router.post('/', function(req, res, next) {
	...
	AWS.cognito.validate(req.cookies.token, function(err, auth) {
		if(err) {
			...
			// TODO: überprüfen das permissions richtiges format hat
			VD.setPermission(auth.username, req.body.forUser, req.body.windowName, JSON.parse(req.body.permissions), function(reply) {
				res.send(reply);
				res.end();
			});
		}
