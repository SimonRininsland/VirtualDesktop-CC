...
\end{itemize}
Es wäre durchaus denkbar fast alle Punkte gut auf einer klassischen, nicht Cloudservice basierenden Serverumgebung umzusetzen. Aber spätestens bei der beliebigen Skalierung stößt man auf Probleme. 
\\
...
Wie bereits in den Anforderungen beschrieben, haben wir uns als Ziel gesetzt den gesamten Service Stack mit einen Befehl ausrollen zu können. Der Befehl mit den Parametern ist folgender:
...
Die Voraussetzung, um unser Stack Deployment Script ausführen zu können ist die Installation von node, aws-sdk, archiver und fs. \\
Mit der Ausführung des Scripts werden die Templates für den Stack und die Lambda Anwendung inklusive der benötigten Dateien in einen \textit{CodeBucket} gestreamt, womit in \textit{Cloudformation} dann die Anwendungen aufgebaut werden. Ein Abbau des Stacks ist ebenso möglich und wird nach dem ausführen des deployment Scripts ausgegeben. \\
...
\caption{Architektur Virtual-Desktop}
\end{figure}
...
Die ElasticBeanstalk Umgebung \textit{AWS::ElasticBeanstalk::Environment} beinhaltet die Konfiguration der eigentlichen Instanz.
\begin{lstlisting}[xleftmargin=\parindent,numbers=left,numberstyle=\small,numbersep=8pt,frame=L,mathescape=true, basicstyle=\small, language=Java, lineskip={1.0pt}]
